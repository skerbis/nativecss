...
| `scroll-stack.css` | `.ncss-stack-cards`, `.ncss-stack-card` | Karten, die sich beim Scrollen aufeinander stapeln (jede neue schiebt sich über die vorherige) statt nur nacheinander einzurasten (siehe `.ncss-snap-sections` weiter oben). Baseline-Mechanismus ist reines `position:sticky` (funktioniert überall, kein JS) - wo `animation-timeline: view()` unterstützt wird (aktuell nicht Baseline, `@supports`-abgesichert), weicht die bedeckte Karte zusätzlich per 3D-Transform (`scale()`/`translateZ()`/`rotateX()`, `perspective` auf `.ncss-stack-cards`) leicht in die Tiefe zurück - ohne Unterstützung bleibt der Baseline-Stapel-Effekt unverändert bestehen. Tokens: `--ncss-stack-perspective` (1200px), `--ncss-stack-card-height` (100vh), `--ncss-stack-scale` (0.88), `--ncss-stack-depth` (-120px), `--ncss-stack-tilt` (8deg), `--ncss-stack-brightness` (0.7). Demo: `demo/scroll-sections.html` |

...
| `tile.css` | `.ncss-tile` (+ `--muted`/`--brand`/`--brand-2`) | Volle-Breite Farbflächen-Sektionen zum Gliedern einer langen Seite (bekannt aus UIkit, `uk-tile-*`). `--brand`/`--brand-2` kippen automatisch auf hellen Text - weiße Flächen darin (`.ncss-card`, `<pre>`) werden automatisch wieder auf normalen dunklen Text zurückgesetzt. Bricht selbst NICHT aus einem `.ncss-container` aus - dafür zusätzlich `.ncss-full-bleed` kombinieren. Padding über `--ncss-tile-padding-block`/`-inline` parametrisierbar (Default `--ncss-space-2xl`/`-lg`) |
